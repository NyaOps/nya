...
pub fn get_nya_context(path: &str) -> Result<NyaContext, String> {
...
fn to_async_context(ctx: ExecutionContext) -> NyaContext {
  Arc::new(Mutex::new(ctx))
}

#[cfg(test)]
mod context_tests {
    use crate::core::context::get_nya_context;

    #[test]
    fn get_nya_context_returns_context() -> Result<(), String> {
      let nya_context = get_nya_context("./context/nya_test_context.json")?;
